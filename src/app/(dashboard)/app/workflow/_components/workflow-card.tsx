...
function WorkflowCard({ workflow }: { workflow: Workflow }) {
  ...
  return (
    <Card className="p-0 border border-separate shadow-sm rounded-lg overflow-hidden hover:shadow-md dark:shadow-primary/30 group/card">
      <CardContent className="p-4 flex items-center justify-between h-full">
          ...
          <div>
            <h3 className="text-base font-bold text-muted-foreground flex items-center">
              <TooltipWrapper content={workflow.description!}>
                <Link href={`/app/workflow/editor/${workflow.id}`}>
                  {workflow.name}
                </Link>
              </TooltipWrapper>
              {isDraft && (
        ...
        <div className="flex items-center space-x-2">
          ...
          <Link
            href={`/app/workflow/editor/${workflow.id}`}
            className={cn(
